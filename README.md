...
| API 챗봇(GPT‑4o) | Top‑K(재시도 로직) 최적화 | 초기 5/20 → 재검색 15/30 채택 (품질·속도 균형, 자체 정량 RAG 평가 총점 18.35/20) |
...
| 데이터 | 규모 | 생성/수집 방식 | 용도 |
|---|---|---|---|
| 구글 API 공식 문서 | **2,000+건** (11개 API, txt) | 웹 크롤링 → 텍스트 추출 | API 챗봇 원문 벡터 DB |
| 구글 API QA셋 | **14,858쌍** (11개 API, JSONL) | 원문 청크 페어 기반 GPT‑4o‑mini 생성 | API 챗봇 QA 벡터 DB |
| 사내 내부 문서 | **60건** (4개 팀 × 15건, 마크다운) | GPT‑4o 프롬프트 합성 | 사내 sLLM 권한별 벡터 DB |
| sLLM 파인튜닝 대화 | **2,368건** (멀티턴 JSON) | 문서 기반 질문 생성 → GPT‑4o 멀티턴 합성 | Qwen3‑8B LoRA 학습 |

> 파인튜닝 데이터는 **업무:일상 = 2:1**, 정중체/반말체 동일 비중, 팀(Backend/Frontend/Data_AI/CTO) 균등 분포로 구성했습니다.

### 🛠️ 기술·서빙 스택(담당 영역 중심)

| 계층 | 구성 |
|---|---|
| **오케스트레이션** | LangGraph / LangChain — API 챗봇 워크플로우, 툴 호출, 품질 평가·재검색 루프 |
| **LLM / sLLM** | GPT‑4o·4o‑mini(OpenAI API), 품질 평가 GPT‑4.1 / Qwen3‑8B LoRA 파인튜닝 |
| **sLLM 서빙** | **RunPod GPU + vLLM**, FastAPI inference API (Nginx 프록시 연동) |
| **검색·저장소** | Chroma(원문/QA 분리) + BGE‑M3 임베딩 + BM25(태그별 인덱스 pkl 캐싱) |
| **애플리케이션/인프라** | Django(Gunicorn/Uvicorn), MySQL, AWS S3(이미지), Elastic Beanstalk + Docker, GitHub Actions CI/CD |
| **평가** | RAGAS + 자체 정량 RAG 평가(LLM‑as‑a‑Judge), TOOL CALL 자동 채점 |

### 🧭 설계·접근 요약(서비스별)

#### API 챗봇 (LangGraph + GPT‑4o)

<img src="images/langgraph1.png" alt="API 챗봇 LangGraph 워크플로우" width="300" />
...
| 평가 | 데이터셋 | 채점자 | 척도 |
|---|---|---|---|
| API 챗봇 RAGAS / 자체 정량 | RAGAS 합성 질문셋 **20문항** (`dataset2.csv`) | GPT‑4.1‑mini | RAGAS 3지표 / 자체 정량 1~5점 × 4항목(20점 만점) |
| API 챗봇 vs Perplexity | 구글 API 질문‑정답 쌍 **20문항** | GPT‑4.1‑mini | 항목당 0~3점 × 3항목 → 100점 환산 |
| sLLM RAGAS | 자체 구축 **55문항** | LLM 평가 | RAGAS 3지표 |
| sLLM 자체 정량 / Top‑K | 자체 구축 질문셋 | LLM 평가 | 항목당 0~3점 × 3항목 → 100점 환산 |
| sLLM TOOL CALL | 멀티턴 툴 호출 테스트셋 | 규칙 기반 자동 채점 | 아래 지표 정의 참조 |

> 버전 비교는 **동일 데이터셋·동일 채점 모델·동일 지표**로 수행했고, LLM 호출은 temperature=0으로 고정해 재현성을 확보했습니다.
...
#### sLLM 자체 정량 RAG 평가(Qwen2.5‑7B vs Qwen3‑8B)

정확도·재현율·구체성을 각 0~3점으로 채점 후 100점 환산한 결과로, **Qwen3‑8B를 최종 모델로 선정한 근거**입니다.
...
> TOOL CALL 지표만 보면 파인튜닝 후 Qwen2.5‑7B가 근소하게 높지만(99.68% vs 98.05%), RAGAS와 자체 정량 평가에서 Qwen3‑8B가 전 지표 우위여서 **RAG 전반 성능을 기준으로 Qwen3‑8B를 채택**했습니다.
...
#### API 챗봇(GPT‑4o) RAG 최적화(재시도 로직 적용)

#### 1단계: 벡터 DB 구조 선정(원문 vs QA vs 하이브리드)

동일 LangGraph 구조에서 검색 대상 DB만 바꿔 비교했습니다.

| 구조 | Context Recall | Faithfulness | Factual Correctness | 해석 |
|---|---:|---:|---:|---|
| 원문 DB만 | 0.6500 | 0.6013 | 0.3475 | 문맥 회수율이 낮으나 정답 일치도는 상대적으로 안정적 |
| QA DB만 | 0.7500 | 0.7205 | 0.3130 | 검색은 개선되나 정답과의 의미적 일치가 가장 낮음 |
| **원문 + QA(하이브리드)** | **0.8000** | 0.6912 | 0.3185 | Recall 최고, 나머지 지표도 균형 |

> 두 DB가 잘 처리하는 질문 유형이 상호 보완적이어서 **하이브리드 구조를 출발점으로 확정**한 뒤, 아래 VER‑1~6 개선을 진행했습니다.

#### 2단계: 개선 과정의 버전별 RAGAS(API 챗봇 VER‑1~VER‑6)
...
> 여기서는 구조가 실제로 달라진 6개 버전만 추렸습니다. 중간 실험까지 모두 포함한 전체 로그(VER‑1~9)는 하단 부록에 있으며, 위 **VER‑6(최종)은 부록의 VER‑9(최종)와 동일한 구성**입니다.
...
| 한계 | 내용 | 대응 / 개선 방향 |
|---|---|---|
| **정확도 ↔ 응답 속도** | 앙상블 검색(유사도+키워드) + 한·영 쿼리 + HyDE 기반 재검색을 쌓으면서 정확도는 올랐지만 응답 지연이 커짐 | 재시도 최대 1회 제한, 태그별 BM25 인덱스 pkl 캐싱으로 완화 / 시맨틱 캐시·리랭킹으로 추가 단축 필요 |
| **Factual Correctness 상대적 열위** | sLLM(Qwen3‑8B)의 Factual Correctness가 0.3709로 Context Recall(0.9273)·Faithfulness(0.8614) 대비 낮음 | 검색은 충분하나 생성 단계의 정답 일치가 약함 → 정답 정합 중심의 학습 데이터 보강 필요 |
| **평가셋의 대표성** | 자체 정량 평가·합성 질문셋을 LLM으로 생성해, 실사용 질의의 다양성·복잡도를 완전히 반영하지 못함 | 실사용 로그 기반 평가셋 구축 필요 |
| **평가 규모** | API 챗봇 버전 비교는 20문항 기준이라 지표 간 소수점 차이를 과대 해석할 위험이 있음 | Top‑K 선정 시 조합 간 차이가 미미했던 RAGAS 대신 자체 정량 평가를 기준으로 삼아 보완 |
| **벡터 DB 최신화 미적용** | 문서 최신화 로직은 개발했으나 시간 제약으로 운영 서비스에 반영하지 못함 | 수집 → 전처리 → 색인 → 배포 자동화 파이프라인으로 이어갈 예정 |

### 📚 배운 점 · 다음 단계

- 데이터/지표/로그 기반으로 성능 개선을 증명하는 습관 — 구조를 바꿀 때마다 동일 데이터셋·동일 채점 모델로 버전을 기록해, 감이 아닌 수치로 선택 근거를 남겼습니다.
- 지표가 서로 어긋날 때(RAGAS vs 자체 정량) 무엇을 최종 기준으로 삼을지 정하는 것이 실제 설계 판단의 핵심이었습니다.

**다음 단계**

| 과제 | 내용 |
|---|---|
| **벡터 DB 최신화 자동화** | 구글 API 문서 수집 → 전처리 → 색인 → 배포까지 자동화하고, 색인 갱신 후 배포·롤백 파이프라인 구축 |
| **임베딩 모델 파인튜닝** | 도메인 용어·문서 특성에 맞춘 임베딩 학습으로 검색 정확도 개선, 재색인 및 재학습 프로세스 정립 |
| **리랭킹 · 시맨틱 캐시** | Cross‑encoder 리랭킹으로 상위 문서 정밀도 보강, 반복 질의는 시맨틱 캐시로 지연·비용 절감 |
| **응답 형식 다양화** | 표/목록/절차/Q&A/요약 등 출력 형식을 JSON 스키마로 정의해 사용자가 선택할 수 있도록 확장 |
| **지원 API 확장** | 구글 외 네이버·카카오 등 외부 API로 검색 대상 확대 및 멀티스텝 워크플로우 구성 |
...
**RAGAS 평가:**
- VER-9(최종)이 Context Recall 0.9250, Factual Correctness 0.5455로 가장 높은 성능 기록
...
- VER-9(최종)은 평균 80.67점으로 높은 점수를 기록하며 첫 실행 안정성 측면에서 유리
...
- **첫 실행 성공률**: VER-9는 첫 실행에서 실패율이 낮아 빠른 응답 속도와 안정적 성능을 동시에 보장
...
3. **도메인 특화 RAG**: Google API 문서 원문/QA셋을 벡터 DB에 넣고, 해당 내용 기반 정확한 RAG 답변
...
| **📊 RAG 시스템의 높은 성능** | LangGraph RAG 모델(VER-9)은 Context Recall 0.9250, Faithfulness 0.8521, Factual Correctness 0.5455의 높은 성능 보임 |
| **🥇 경쟁 우위 확보** | LangGraph RAG 시스템은 Perplexity 모델 대비 종합 평균 21.11%p의 우위를 입증                                                  |
| **🔧 다양한 기능 구현** | 멀티턴 대화, 도메인 특화 답변, 권한 기반 문서 접근 제어, 대화 카드 저장, 커뮤니티 기능 등 실제 활용 가능한 기능들 성공적으로 완성                             |
...
| **🔄 LangGraph의 답변 속도 최적화 한계** | API 챗봇 LangGraph에서 앙상블 검색(유사도+키워드), 한·영 쿼리 검색, 쿼리 재생성 및 재검색 후 다시 답변하는 로직(HyDE 가상 답변 기법) 적용으로 정확도는 높아졌으나 답변 속도가 느려짐 |
